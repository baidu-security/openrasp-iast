...
class ScanPlugin(scan_plugin_base.ScanPluginBase):
    ...
    plugin_info = {
        "name": "eval_basic",
        "show_name": "PHP代码执行检测插件",
        "description": "PHP eval代码执行漏洞检测插件"
    }
    ...
    def mutant(self, rasp_result_ins):
        ...
        测试向量生成
        """
        if not rasp_result_ins.has_hook_type("eval"):
            return

        payload_list = [("str()'\"$openrasp", "str()'\"$openrasp")]
        ...
        for param in test_params:
            if not request_data_ins.is_param_concat_in_hook("eval", param["value"]):
                continue
    ...
    def check(self, request_data_list):
        ...
        if rasp_result_ins is None:
            return None
        if self.checker.check_concat_in_hook(rasp_result_ins, "eval", feature):
            return "eval执行的代码可被用户输入控制"
        else:
            return None
