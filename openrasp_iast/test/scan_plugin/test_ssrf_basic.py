...
from plugin.scanner import ssrf_basic
...
def get_normal_response():
    ...
    rasp_result_json = """
    {
        ...
        "context": {
            ...
            "protocol": "http", 
            "method": "get", 
            "querystring": "url=file:///etc/passwd", 
            ...
            "url": "http://localburp.com:8005/011-ssrf-curl.php?url=file:///etc/passwd", 
            "nic": [
                {
                    "name": "eth0", 
                    "ip": "172.17.0.2"
...
async def send_request(self, request_data):
    url = request_data.get_param("get", "url")
    
    if url is not None and url.find("127.1.2.3") >= 0:
...
def scan_plugin_fixture():
    ...
    plugin_ins = ssrf_basic.ScanPlugin()
